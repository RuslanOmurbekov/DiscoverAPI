...
public class GoogleMapsTest_stepDefs {
	...
	public void user_validates_UI_data_to_API_data() throws Throwable {
	   String apidistance=WebServiceRunner.getResponse().getRoutes().get(0).getLegs().get(0).getDistance().getText();
	   String apiduration=WebServiceRunner.getResponse().getRoutes().get(0).getLegs().get(0).getDuration().getText();
	   Driver.getInstance().findElement(By.xpath("//*[@id='directions-searchbox-1']/button[1]")).click();
	   String uiduration=Driver.getInstance().findElement(By.xpath("//div[@class='section-directions-trip-duration delay-heavy']")).getText();
	  String uidistance= Driver.getInstance().findElement(By.xpath("//div[@class='section-directions-trip-distance section-directions-trip-secondary-text']")).getText();
	  //String uiduration1=uiduration.substring(0,6);
	  String uidistance1= uidistance.substring(0,6);
	  Assert.assertEquals(apidistance, uidistance1);
	  Assert.assertEquals(apiduration, uiduration);
	}
}
